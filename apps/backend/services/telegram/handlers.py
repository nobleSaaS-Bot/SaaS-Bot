...
async def _upsert_customer(ctx: TenantBotContext, tg_user: dict) -> None:
...
async def on_start(ctx: TenantBotContext, message: dict[str, Any], payload: str = "") -> None:
...
async def on_message(ctx: TenantBotContext, message: dict[str, Any]) -> None:
    ...
    async with async_session_factory() as db:
        ...
        if session:
            ...
            await send_welcome_message(ctx, chat_id)


async def on_callback_query(ctx: TenantBotContext, callback_query: dict[str, Any]) -> None:
...
async def on_pre_checkout_query(ctx: TenantBotContext, query: dict[str, Any]) -> None:
    """Approve all pre-checkout queries (payment step)."""
    await ctx.answer_pre_checkout(query["id"], ok=True)


async def on_successful_payment(ctx: TenantBotContext, message: dict[str, Any]) -> None:
